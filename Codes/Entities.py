...
with open("grouped_by_community.json", "r") as f:
    data = json.load(f)

# Example list of specific DIDs to analyze
specific_dids = [
...
# Calculate unique interactions for each DID
unique_entities = defaultdict(set)

for community, transactions in data.items():
    for tx in transactions:
        source = tx["sourceDID"]
        target = tx["targetDID"]
        if source in specific_dids:
            unique_entities[source].add(target)
        if target in specific_dids:
            unique_entities[target].add(source)

# Prepare data for plotting
did_labels = []
interaction_counts = []

for did in specific_dids:
    did_labels.append(did)
    interaction_counts.append(len(unique_entities[did]))

# Combine data into a list of tuples for sorting
sorted_data = sorted(
    zip(interaction_counts, did_labels), 
    reverse=True  # Sort by counts in descending order
)
sorted_data = sorted_data[:20]  # Select the top 20
# Unzip the sorted data
sorted_counts, sorted_labels = zip(*sorted_data)

# Convert data to a format suitable for Seaborn
plot_data = {"DIDs": sorted_labels, "Unique Interactions": sorted_counts}

# Plot the results using Seaborn
plt.figure(figsize=(10, 8))
sns.barplot(
    x="Unique Interactions", 
    y="DIDs", 
    data=plot_data, 
    palette="Blues_r"
)
plt.xlabel("Count of Unique Entities", fontsize=12)
plt.ylabel("DIDs", fontsize=12)
plt.title("Unique Interactions Per DID", fontsize=14)
plt.tight_layout()
plt.show()