...
# Counter to track operations and their targets
...
for community_id, interactions in data.items():
    for interaction in interactions:
        ...
        # Increment the counter for the operation -> target combination
        operation_target_counter[(operation_type, target)] += count

# Debugging: Print the operation-target counter to verify results
print("Operation-Target Counter:", operation_target_counter)

# Get the top 10 operation-target pairs by count
top_operations_targets = operation_target_counter.most_common(20)

# Prepare data for plotting
operation_target_labels = [
    f"{op}\n({target})" for op, target in [pair[0] for pair in top_operations_targets]
]
counts = [pair[1] for pair in top_operations_targets]
...
sns.barplot(x=counts, y=wrapped_labels, palette="viridis")

# Add titles and labels
plt.title("Top 10 Operations by Targets Performed by Specific DIDs", fontsize=18)
plt.xlabel("Total Count", fontsize=16)
plt.ylabel("Operation (Target)", fontsize=16)
